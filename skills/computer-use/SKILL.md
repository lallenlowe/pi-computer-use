...
- Coordinates for `click`/`double_click`/`move_mouse`/`drag`/`scroll` are **window-relative logical points** (top-left origin), in `[0, windowWidth]` × `[0, windowHeight]` — the same units as the `framePoints.w/h` returned by `list_windows`. Screenshots are normalized to logical-point resolution (1 image pixel == 1 window logical point), so **pixel coords you read off the image are the same numbers you pass to click/move/drag/scroll** — no scale math. The screenshot envelope's `Coords:` line confirms this every call. Rare exception: when the window's logical size exceeds the per-edge image cap, the helper downscales further and the envelope reports `scale < 1`; divide by that scale in that case.
...
- `select_text({ ref, text })` selects a literal substring inside an AX text element by content, no coords. Use it instead of click+arrow-keys to position the cursor or select a range when the target text is in the AX tree. Pass `prefix`/`suffix` (surrounding chars) to disambiguate when the text appears multiple times. `placement: "before"|"after"` places the caret without selecting.
